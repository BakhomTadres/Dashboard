  ...
  return (
    <>
      ...
      <aside className="fixed pt-8 h-[calc(100vh-64px)] top-15 w-[calc(75%-32px)] lg:w-[30%] bg-white md:px-4">
        {isRegistered && (
          <>
            <div className="flex flex-col lg:flex-row justify-start items-center lg:space-x-4 space-x-2 space-y-2 lg:space-y-0 p-4 bg-mauve-100">
              <div
                onClick={() => setShowEditProfile(true)}
                className="flex items-center justify-center cursor-pointer w-10 h-10 p-1 rounded-full bg-teal-400 text-white font-bold text-2xl relative"
              >
        ...
        {isRegistered && (
          <button
            onClick={async () => {
              ...
              navigate("/login");
              await axios.post(
                "https://dashboard-backend-ebon.vercel.app/api/users/logout",
                {},
                {
                  headers: {
